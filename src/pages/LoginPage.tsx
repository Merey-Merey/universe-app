...
const LoginPage: React.FC = () => {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const credential = await signIn(form.email.trim(), form.password);
      await hydrateUser(credential.user.uid, credential.user);
      navigate("/home");
    } catch {
      setAuthError("Could not sign in with this email and password.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="page-wrapper mobile-only">
        ...
        <div className="auth-body">
          ...
          <div className="auth-actions">
            <button className="primary-btn" onClick={submit} disabled={loading || googleLoading}>
